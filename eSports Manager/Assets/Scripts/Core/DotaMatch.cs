...
public class DotaMatch : MonoBehaviour
{
    ...
    private float CalculateBotLaneEGResult()
    {
        ...
        botLaneEGResult = (rcVSdo + rhsVSds + rcVSds + rhsVSdo + (teamVSteam*teamFactorMultiplicator)) / 5;
    ...
    private float CalculateFloatResultOfAdvantages(float team1advantage, float team2advantage)
    {
        ...
        bool isResultPositive = CalculateResultPositiveNegative(resultCalculation);

        if (isResultPositive)
        {
            endResult = result - (resultCalculation * 0.005f);
        } else
        {
            endResult = result + (resultCalculation * 0.005f);
        }
